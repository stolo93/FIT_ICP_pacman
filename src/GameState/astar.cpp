...
struct hash<game::PathNode> {
    ...
    {
        return (hash<float>()(node.position.x.to_floating_point<float>()) ^
                hash<float>()(node.position.y.to_floating_point<float>()) << 3);
    }
};
}    // namespace std
...
PathNode::PathNode() : position(), g(), h(), f() {}


PathNode::PathNode(Pos position, double g, double h) : position(position), g(g), h(h), f(g + h) {}

PathNode::PathNode(const PathNode &other) noexcept
    : position(Pos(other.position.x, other.position.y)), g(other.g), h(other.h), f(other.f)
{
}
...
std::vector<PathNode> PathNode::generate_neighbours(const QExplicitlySharedDataPointer<Map> &map, const Pos &goal) const
{
    ...
    for (const auto &direction : DIRECTIONS) {
        ...
        if (is_node_valid(map, new_node)) { neighbours.emplace_back(new_node); }
    }
...
{
    if (node.position.x < 0 || node.position.y < 0 || node.position.x >= map->get_width() ||
        node.position.y >= map->get_height()) {
        return false;
    }
...
std::vector<Pos> A_Star::a_star(const QExplicitlySharedDataPointer<Map> &map, const game::Pos &start,
                                const game::Pos &goal)
{
    auto node_compare = [](const PathNode &a, const PathNode &b) -> bool {
        if (a.f == b.f) {
            return a.h > b.h;
        } else {
            return a.f > b.f;
...
std::vector<Pos> A_Star::reconstruct_path(const PathNode &start, const PathNode &goal,
...
std::vector<Pos> A_Star::dumb_move(const QExplicitlySharedDataPointer<Map> &map, const game::Pos &start,
                                   const game::Pos &goal)
{
    auto node_compare = [](const PathNode &a, const PathNode &b) -> bool {
        if (a.f == b.f) {
    ...
    if (neighbours.empty()) { return {start, start}; }

    PathNode min_dist = neighbours[0];
    for (int i = 1; i < neighbours.size(); ++i) {
        if (! node_compare(neighbours[i], min_dist)) { min_dist = neighbours[i]; }
    }
